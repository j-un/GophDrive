...
describe("linkifyTags", () => {
  it("linkifies a basic inline tag", () => {
    ...
    expect(result).toContain(
      '<a href="/notes?tag=dev" class="tag-link">#dev</a>',
    );
  });
  ...
  it("skips content inside <code> blocks", () => {
    ...
    expect(result).not.toContain('href="/notes?tag=notag"');
    expect(result).toContain('href="/notes?tag=realtag"');
  });
  ...
  it("skips content inside <pre> blocks", () => {
    ...
    expect(result).not.toContain('href="/notes?tag=inside"');
    expect(result).toContain('href="/notes?tag=outside"');
  });
  ...
  it("trims trailing slashes from tag name", () => {
    ...
    expect(result).toContain('href="/notes?tag=work"');
  ...
  it("tag after Japanese punctuation", () => {
    ...
    expect(result).toContain(
      'href="/notes?tag=%E9%96%8B%E7%99%BA%E3%81%99%E3%82%8B"',
    );
  });
